...
All code for this project was developed and executed in **Google Colab** with GPU acceleration, as the pipeline involves large-scale graph construction, GNN training, evidence extraction, and LLM-based explanation generation that depend on a continuous runtime with shared memory.
```
notebooks/
├── full-pipeline/
│   └── DS_5500_Codebook_Comprehensive.ipynb
├── modules/
│   ├── 1.Environment_setup.ipynb
│   ├── 2.Data_loader.ipynb
│   ├── 3.Preprocessing_pipeline+Hetro...ipynb
│   ├── 4.EDA_visualizations.ipynb
│   ├── 5.Train_Val_Test_split+PyG_Het...ipynb
│   ├── 6.Model_Baseline_GNN+Model_...ipynb
│   ├── 7.Model_evaluation_metrics.ipynb
│   ├── 8.Re-rank+evidence+safety+simi...ipynb
│   ├── 9.run_demo()_pipeline.ipynb
│   └── A.Demo_Sample_demo_output...ipynb
└── README.md
```

The primary runnable notebook is:

**`notebooks/full-pipeline/DS_5500_Codebook_Comprehensive.ipynb`** — This is the single end-to-end notebook that contains the full pipeline from data loading through model training, evaluation, re-ranking, and RAG-based explanation generation. **To reproduce results, run this notebook on Google Colab with a GPU runtime.**
...
| `3.Preprocessing_pipeline+Hetro...` | Data cleaning, normalization, and heterogeneous graph construction |
| `4.EDA_visualizations` | Exploratory data analysis and graph statistics |
| `5.Train_Val_Test_split+PyG_Het...` | Train/validation/test edge splits and PyG HeteroData preparation |
| `6.Model_Baseline_GNN+Model_...` | Baseline heterogeneous GNN and HGT model training |
| `7.Model_evaluation_metrics` | Classification, ranking, and safety metric evaluation |
| `8.Re-rank+evidence+safety+simi...` | Evidence extraction, safety filtering, disease similarity, and re-ranking |
| `9.run_demo()_pipeline` | End-to-end demo pipeline for a given disease query |
| `A.Demo_Sample_demo_output...` | Sample output showing predicted drugs, evidence paths, and explanations |
